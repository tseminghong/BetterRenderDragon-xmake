...
static ImGui_ImplWinRT_Data *ImGui_ImplWinRT_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplWinRT_Data *)ImGui::GetIO().BackendPlatformUserData
             : nullptr;
}
...
static ImGuiKey
ImGui_ImplWinRT_VirtualKeyToImGuiKey(ABI::Windows::System::VirtualKey vk) {
...
#define RETURN_IF_FAILED(hr)                                                   \
  {                                                                            \
    HRESULT __hr_##__LINE__##__ = (hr);                                        \
    if (FAILED(__hr_##__LINE__##__)) {                                         \
      return;                                                                  \
    }                                                                          \
  }
#define WARN_IF_FAILED(hr)                                                     \
  {                                                                            \
    HRESULT __hr_##__LINE__##__ = (hr);                                        \
    if (FAILED(__hr_##__LINE__##__)) {                                         \
      printf("%s Line %d FAILED hr=0x%08X\n", __FUNCTION__, __LINE__,          \
             __hr_##__LINE__##__);                                             \
    }                                                                          \
  }

ImGuiInputEventHandler::ImGuiInputEventHandler(ICoreWindow *window)
    : window(window) {
  ...
  typedef ABI::Windows::Foundation::ITypedEventHandler<
      ...
      CharacterReceivedEventHandler;

  WARN_IF_FAILED(this->window->add_PointerMoved(
      Callback<PointerEventHandler>(this,
                                    &ImGuiInputEventHandler::onPointerMoved)
          .Get(),
      &pointerMovedToken));
  WARN_IF_FAILED(this->window->add_PointerExited(
      Callback<PointerEventHandler>(this,
                                    &ImGuiInputEventHandler::onPointerExited)
          .Get(),
      &pointerExitedToken));
  WARN_IF_FAILED(this->window->add_PointerPressed(
      Callback<PointerEventHandler>(this,
                                    &ImGuiInputEventHandler::onPointerPressed)
          .Get(),
      &pointerPressedToken));
  WARN_IF_FAILED(this->window->add_PointerReleased(
      Callback<PointerEventHandler>(this,
                                    &ImGuiInputEventHandler::onPointerReleased)
          .Get(),
      &pointerReleasedToken));
  WARN_IF_FAILED(this->window->add_PointerWheelChanged(
      Callback<PointerEventHandler>(
          this, &ImGuiInputEventHandler::onPointerWheelChanged)
          .Get(),
      &pointerWheelChangedToken));
  WARN_IF_FAILED(this->window->add_KeyDown(
      Callback<KeyEventHandler>(this, &ImGuiInputEventHandler::onKeyDown).Get(),
      &keyDownToken));
  WARN_IF_FAILED(this->window->add_KeyUp(
      Callback<KeyEventHandler>(this, &ImGuiInputEventHandler::onKeyUp).Get(),
      &keyUpToken));
  WARN_IF_FAILED(this->window->add_CharacterReceived(
      Callback<CharacterReceivedEventHandler>(
          this, &ImGuiInputEventHandler::onCharacterReceived)
          .Get(),
  ...
  displayInfo->add_DpiChanged(
      Callback<
          ...
          this, &ImGuiInputEventHandler::onDpiChanged)
          .Get(),
      &dpiChangedToken);
}
...
void ImGuiInputEventHandler::updateMouseButtonState(IPointerEventArgs *args) {
  ...
  io.AddMousePosEvent(ConvertDipsToPixels(pos.X, dpi),
                      ConvertDipsToPixels(pos.Y, dpi));
...
HRESULT ImGuiInputEventHandler::onPointerMoved(ICoreWindow *sender,
...
HRESULT ImGuiInputEventHandler::onPointerExited(ICoreWindow *sender,
                                                IPointerEventArgs *args) {
  auto &io = ImGui::GetIO();
...
HRESULT ImGuiInputEventHandler::onPointerPressed(ICoreWindow *sender,
                                                 IPointerEventArgs *args) {
  updateMouseButtonState(args);
  return S_OK;
}

HRESULT ImGuiInputEventHandler::onPointerReleased(ICoreWindow *sender,
                                                  IPointerEventArgs *args) {
  updateMouseButtonState(args);
  return S_OK;
}

HRESULT ImGuiInputEventHandler::onPointerWheelChanged(ICoreWindow *sender,
                                                      IPointerEventArgs *args) {
  auto &io = ImGui::GetIO();
...
HRESULT ImGuiInputEventHandler::onKeyDown(ICoreWindow *sender,
                                          IKeyEventArgs *args) {
  auto &io = ImGui::GetIO();
...
HRESULT ImGuiInputEventHandler::onKeyUp(ICoreWindow *sender,
                                        IKeyEventArgs *args) {
  auto &io = ImGui::GetIO();
...
HRESULT
ImGuiInputEventHandler::onCharacterReceived(ICoreWindow *sender,
...
HRESULT ImGuiInputEventHandler::onDpiChanged(
    ABI::Windows::Graphics::Display::IDisplayInformation *sender,
    IInspectable *args) {
  printf("DPI changed\n");
